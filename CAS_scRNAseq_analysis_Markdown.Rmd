...
Now run the function to apply preprocessing to the raw data. The function will create several output plots and save them in the QC directory.

Optional; can also save a seurat object of the preprocessed data (if save.obj = "TRUE). Note: 'save.obj' is set to 'FALSE' as the seurat object saved is > 100mb for each sample, which is larger than github's limit.
...
```{r, warning=FALSE, message=FALSE}
setwd(Out.dir2)
png("2a)_UMAP_celltype.png", units="in", width=6, height=5, res=300)
...
invisible(dev.off())
png("2b)_UMAP_celltype_label.png", units="in", width=6, height=5, res=300)
DimPlot(s.obj, reduction = "umap", group.by = "Azimuth_2_CT", label = T,
        repel=T, cols = set.col) + NoLegend()
invisible(dev.off())
```
...
sx = subset(sx, (Azimuth_2_CT == c("CD14 Mono")))
...
Plot by partition (major, non-connection clusters)
```{r, warning=FALSE, message=FALSE, fig.align='center'}
...
plot_cells(cds3, genes = "IRF1")
```

Plot by age and stimuli
```{r}
...
```{r, warning=FALSE, message=FALSE, eval=TRUE, echo=FALSE}
setwd("C:/Users/jread/Desktop/Work/CAS_scRNAseq/New_Cleaned_data")
# saveRDS(cds4, "cds_mono_ordered.rds")
cds4 = readRDS("cds_mono_ordered.rds")
```


### Plot

Plot by age and stim
```{r, warning=FALSE}
setwd(Out.dir2)
...
Add monocle results to subsetted seurat object seurat object
...
```{r, message=FALSE, warning=FALSE}
setwd(Out.dir2)
png("14)_LPS_CBMC_Circos.png", width=7, height=6, units = "in", res=600)
...
```{r, warning=FALSE, message=FALSE}
setwd(Out.dir2)
png("15)_LPS_CBMC_CD4T_Mono_sankey.png", width=6, height=5, units = "in", res=600)
